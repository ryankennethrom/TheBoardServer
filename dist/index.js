...
function drawStrokeToCanvas(strokeMsg, ctx, drawLine) {
    var { prevPoint, currentPoint, color } = JSON.parse(strokeMsg.content.toString());
    if (!ctx)
        return;
    drawLine({ prevPoint, currentPoint, ctx, color });
}
function drawImageToCanvas(base64Image, ctx) {
...
function drawStrokesToCanvas(strokes, ctx) {
...
const sendLineToQueue = ({ prevPoint, currentPoint, color }, channel) => {
...
const sendImageToQueue = (base64Image, channel) => {
    let queueName = "drawQueue";
    channel.assertQueue(queueName, {
        durable: false,
    });
...
const sendStrokesToQueue = (queue, channel) => {
